async function getAiMatches(pet, sightings) {
  ...
  if (top && top.score >= 80 && scoreGap > 10) {
    return {
      matches: [
        createMatch(
          top,
  ...
  if (candidates.length === 1 && top.score >= 120) {
    // ✅ Lowered from 140 to 120
    return {
      matches: [
        createMatch(
          top,
  ...
  // Multiple candidates
  const needsAi =
    top && top.score < 150 && candidates.length > 1 && scoreGap < 30; // ✅ Lowered from 180 to 150
  if (!needsAi) {
    return {
      matches: candidates
        .slice(0, 3)
